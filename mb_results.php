...
RESULTS;
	...
	$result = mysqli_query($connect, "SELECT MBTI from $table where Username = \"$un\"");
        $row = $result->fetch_row();

	if(count($row) == 0){	
		$stmt="INSERT INTO $table (MBTI) VALUES ('$str') WHERE Username = '$un'";
        	$connect->query($stmt);
	}else{
		$stmt="UPDATE $table SET MBTI='$str' WHERE Username='$un';";
                $connect->query($stmt);
	}
        mysqli_close($connect);
	}
