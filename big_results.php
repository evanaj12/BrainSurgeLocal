...
RESULTS;
	...
	function send_results(){
        ...
        $result = mysqli_query($connect, "SELECT Big5 from $table where Username = \"$un\"");
        $row = $result->fetch_row();

        if(count($row) == 0){
                $stmt="INSERT INTO $table (Big5) VALUES ('$str') WHERE Username = '$un'";
                $connect->query($stmt);
        }else{
                $stmt="UPDATE $table SET Big5='$str' WHERE Username='$un';";
                $connect->query($stmt);
        }
        mysqli_close($connect);
}
