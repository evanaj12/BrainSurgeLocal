...
// Get data from a table in the database and print it out
	...
	$name = $_COOKIE["first"];
        ...
        $result_mb = mysqli_query($connect, "SELECT (MBTI) from $table WHERE Username = '$un'");
	...
	$p=strval($mb_strs[8]);
        ...
        $result_big = mysqli_query($connect, "SELECT (Big5) from $table WHERE Username = '$un'");
	...
	$nn=strval($big_strs[3]);
        ...
        $result_ab = mysqli_query($connect, "SELECT (TypeAB) from $table WHERE Username = '$un'");
