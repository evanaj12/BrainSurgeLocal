...
RESULTS;
	...
	function send_results(){
        ...
        $result = mysqli_query($connect, "SELECT TypeAB from $table where Username = \"$un\"");
        $row = $result->fetch_row();

        if(count($row) == 0){
                $stmt="INSERT INTO $table (TypeAB) VALUES ('$str') WHERE Username = '$un'";
                $connect->query($stmt);
        }else{
                $stmt="UPDATE $table SET TypeAB='$str' WHERE Username='$un';";
                $connect->query($stmt);
        }
        mysqli_close($connect);
}
